...
case class Room(dirtPatches: Seq[Position], width: Int, height: Int, instructions: Seq[Instruction])
...
object Room {
  ...
  private def validateInput(data: String): Boolean = {
    ...
    if(lines.nonEmpty){
      ...
      val numberTuples = lines.init.forall{ line =>
        ...
        (maybeInts.size == 2) && maybeInts.forall{maybeInt => Try(maybeInt.toInt).isSuccess}
      }
  ...
  def create(data: String): Option[Room] = {
    if(validateInput(data)) {
      val lines = data.split("\n")
      val linesAsTuples = lines.map { line =>
        val ints = line.split(" ").map(_.toInt)
        ints(0) -> ints(1)
      }
      val (width, height) = linesAsTuples.head
      val patches = linesAsTuples.tail.init.map(Function.tupled(Position.apply))
      val instructions = lines.last.map(Instruction.apply)
      val room = Room(patches, width, height, instructions)
      Some(room)
    } else {
      None
    }
